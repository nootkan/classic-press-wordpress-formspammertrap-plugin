...
* Signup verification processing only runs when `FST_CONTACT_VERIFY` is enabled
* Verification IDs are restricted to the exact FormSpammerTrap GUID format before database lookup
* Contact database connections are validated as real `mysqli` objects before queries are run
* Database query results are checked before accessing `num_rows` or other result methods
* Database sanity checks verify that the configured contact table exists and handle failed `DESCRIBE` queries safely
...
= September 20, 2026 - Security and Database Hardening =
* Hardened `includes/formspammertrap-contact-functions.php` database and signup-verification handling
* Signup verification via the `id` query parameter now runs only when `FST_CONTACT_VERIFY` is enabled
* Added strict validation for FormSpammerTrap verification GUIDs before they are used in database queries
* Added `mysqli` connection-type validation before database queries in both contact-save and signup-verification paths
* Added failed-query checks before accessing `num_rows`
* Corrected the database sanity check to inspect the query result rather than `mysqli_connect_error()` after a query
* Database sanity checking now confirms that the configured contact table actually exists
* Added safe failure handling for `DESCRIBE` queries before calling `fetch_all()`
* PHP syntax checks passed after each change and normal form email delivery was verified after the update

= August 29, 2025 - PHP Compatibility and PHPMailer Loading =
* Replaced invalid `FILTER_VALIDATE_BOOL` references with the correct `FILTER_VALIDATE_BOOLEAN` constant
* Updated the affected validation calls for upload deletion, email checking, and contact verification
* Added conditional PHPMailer loading so the plugin only loads its bundled PHPMailer classes when the PHPMailer class is not already available
* This avoids duplicate PHPMailer class-loading conflicts on WordPress installations that already provide PHPMailer
* Plugin functionality remains unchanged while PHP warnings and intermittent PHPMailer-related critical errors are avoided
...
**PHP / PHPMailer Compatibility Maintenance:**
* Uses the valid PHP constant `FILTER_VALIDATE_BOOLEAN` for boolean validation
* Bundled PHPMailer class files are loaded only when `PHPMailer\\PHPMailer\\PHPMailer` is not already available
* Conditional PHPMailer loading reduces duplicate-class conflicts with WordPress installations that already include PHPMailer
* The plugin's bundled PHPMailer files remain available as the fallback when the class has not already been loaded

**Database and Verification Hardening:**
* Signup verification processing is gated by the `FST_CONTACT_VERIFY` setting
* Verification IDs must match FormSpammerTrap's generated GUID format before database use
* Callers of the contact database connection helper verify that a valid `mysqli` object was returned before querying
* `SELECT` results are checked for failure before `num_rows` is accessed
* Database sanity checking validates both query success and actual table existence
* `DESCRIBE` query failures are handled before `fetch_all()` is called
